...
class ErShouSpider(BaseSpider):
    def collect_area_ershou_data(self, city_name, area_name, fmt="csv"):
        ...
        xlsx_file = self.today_path + \
            "/{0}_{1}.xlsx".format(district_name, area_name)
        ...
        ershous.to_excel(xlsx_file, index=False)
    ...
    def get_area_ershou_info(city_name, area_name):
        ...
        # # 创建一个空的DataFrame，列根据ErShou类的属性设定
        columns = ['district', 'area', 'community_name',
                   'name', 'price', 'unitprice', 'description', 'pic']
        ...
        for num in range(1, total_page + 1):
            ...
            for house_elem in house_elements:
                ...
                # 获取总价部分
                total_price_span = house_elem.find(
                    "div", class_="totalPrice totalPrice2").find("span")
                total_price_value = total_price_span.text.strip()
                total_price_unit = total_price_span.find_next_sibling(
                    "i").text.strip()
                ...
                pic = house_elem.find('a', class_="img").find(
                    'img', class_="lj-lazy")
                community_name = house_elem.find('div', class_='positionInfo')

                # 继续清理数据
                # price = price.text.strip()
                price = total_price_value + total_price_unit
                unitprice = unitprice.text.strip().replace(",", "")
                name = name.text.replace("\n", "")
                desc = desc.text.replace("\n", "").strip()
                desc = re.sub(' +', ' ', desc)
                pic = pic.get('data-original').strip()
                community_name = community_name.text.strip()
                # print(pic)

                # 作为对象保存
                # ershou = ErShou(chinese_district, chinese_area, community_name,
                #                 name, price, unitprice, desc, pic)
                ...
                df_list.append(pd.DataFrame([row_data], columns=columns))
